...
export default function DashboardPage() {
  ...
  const errorConnectors = (connectors ?? []).filter((c: any) => c.status === 'ERROR').length;
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          Resumen del sistema de licitapp unificado
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title="Total licitapp"
        ...
        <StatCard
          title="Con Errores"
          value={errorConnectors > 0 ? String(errorConnectors) : '0'}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {statsLoading ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={bySourceData} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
                ...
                <Tooltip formatter={(v) => [v.toLocaleString('es-CL'), 'licitapp']} />
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
          ...
          {statsLoading ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                <Pie data={byStatusData} dataKey="count" nameKey="name" cx="50%" cy="50%" outerRadius={90} label>
                  {byStatusData.map((_, i) => (
                    <Cell key={i} fill={['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#6b7280'][i % 5]} />
                  ))}
                </Pie>
                <Tooltip formatter={(v) => [v.toLocaleString('es-CL'), 'licitapp']} />
                <Legend />
